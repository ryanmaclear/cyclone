...
contextBridge.exposeInMainWorld('cyclone', {
  ...
  setSerialEmulatorEnabled: (enabled: boolean) => ipcRenderer.invoke('serial:set-emulator-enabled', enabled),
  ...
  sendManualGCode: (command: string) => ipcRenderer.invoke('serial:send-manual-gcode', command),
